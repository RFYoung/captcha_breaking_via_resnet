...
class ResNetModel(nn.Module):
    def __init__(self, char_classes,str_len ,input_shape=(3, 64, 128)):
        ...
        for i, (stride, net_channel_no_expand, num_blocks) in enumerate((zip(resnet_stride,channels, basic_blocks_list))) :
            ...
            features_modules[f'basic-{i}-0'] = FeatureBasicBlock(in_channel,net_channel_no_expand,stride,expansion,shortcut_downsample)
            
            
            for j in range(1,num_blocks):
                features_modules[f'basic-{i}-{j}'] = FeatureBasicBlock(net_channel_no_expand*expansion,net_channel_no_expand,1,expansion,shortcut_downsample=None)
